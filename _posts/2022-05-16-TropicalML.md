...
If interpreted over the tropical semiring polynomials are piecewise linear functions, where the "pieces" - or *linear regions* - are polyhedral. Let us consider the univariate tropical polynomial $$p(x) = x^2 \oplus 2 \odot x \oplus 1$$, in this case the linear regions are the intervals $$(-\infty,-1], [-1,2], [2,+\infty)$$. Here the values $$-1$$ and $$2$$ play a special role, as each of them belongs to two intervals. Moreover, they appear explicitly if we rearrange $$p$$ as $$(x \oplus -1) \odot (x \oplus 2)$$. It makes then sense to call them *tropical roots* of $$p$$, in analogy with what happens in the traditional settings. Note that, as in the example below, the factorizations (and therefore the roots) in the tropical and classical settings are different.
...
And here is the algebraic tropical *quadric* (curve of degree two) it defines.
